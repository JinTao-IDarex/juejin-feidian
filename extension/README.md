...
### 收起态停在哪：右上角内缩位（v0.2.3）

```js
const FOLD_INSET_RIGHT = 164;   // 宿主右缘距视口右边缘
const FOLD_INSET_TOP   = 66;    // 宿主顶边距视口顶边
```

口径：缩略卡是站点自己的 `.dock`，在 196×236 的 iframe 里 `top:20; right:24`、尺寸 148×196。
所以「缩略卡距视口右 188、距顶 86」换算到宿主盒子要**减去**那 24 / 20
（校验：旧值「卡距右 36」→ 宿主 `36−24 = 12` = 原来的 `MARGIN` ✓）。

为什么要内缩：贴角会让缩略卡压在掘金头部右上角的图标/悬浮工具条上。
用户在自己的浏览器上标了目标位置，据此换算成 **相对原位置左移 152px、下移 54px**。

两侧都留了 `Math.max(MARGIN, …)` 兜底，顶部再跟 `vp.h − MARGIN − h` 取小 ——
窄视口 / 矮视口下缩略卡不会跑到看不见的地方。

**实测**（1920×960，`clientWidth` 1905）：宿主 `[1545, 66, 196, 236]`、
缩略卡 `[1569, 86, 148, 196]`、**`dockRightInset = 188`** ✓；
展开⇄收起两向逐帧仍单调无跳变（收起：`1609,139 → 1550 → 1512 → 1503 → 1501`
（卡片长大，右缘不动）→ `1509 → 1552 → 1567 → 1569,86`（窗口位移））。
...
| 收起态几何 | 宿主 `[1545,66,196,236]`、卡 `[1569,86,148,196]`、`dockRightInset = 188` ✓ |
...
| `content.js` | 在 juejin.cn 注入宿主与遮罩、落位、同步展开/收起尺寸、收起态投影（父页面 `filter: drop-shadow` + hover 档） | 手写 |
...
- **缩略卡投影挪到父页面（v0.2.6 修）**：收起态 iframe 读回
  ...
  卡 `[1569,86,148,196]`、`dockRightInset = 188`；展开 `[233,89,1440,722]`
...
- **收起态位置（v0.2.3 定）**：1920×960（`clientWidth` 1905）下宿主 `[1545, 66, 196, 236]`、
  缩略卡 `[1569, 86, 148, 196]`、`dockRightInset = 188` ✓（目标值 188）。
  窄/矮视口有 `Math.max(MARGIN, …)` 兜底。
- **缩略卡背景色（v0.2.5 回滚）**：`#juejin-pin-gacha-frost` 不再存在；
  ...
  几何与两态与 v0.2.3 完全一致（`dockRightInset = 188`；收起 `mask null`、
  展开 `mask [block,1,Z−1]` + 宿主 `1440×722 @(233,119)`）。
