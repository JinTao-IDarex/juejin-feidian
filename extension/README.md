...
> ⚠️ 「一键评论」暂时下线（v0.2.15 起）：掘金网关对代发请求有 CSRF 拦截
> ——隔离世界（content script）直发的 POST 会被吃成 200 空响应、不带 CORS
> 头，浏览器报成跨域失败；v0.2.14 改为注入页面主世界执行虽可通，但稳妥起见
> 先下线观察。下线期间 AI 卡按钮一律为「复制点评」，点击只复制。
> 代发链路（登录探测、主世界桥）原样保留，要恢复改 `site/app.js` 里的
> `COMMENT_POST_ENABLED = true` 即可。
